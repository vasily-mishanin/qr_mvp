...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 flex flex-col-reverse">
      <section className="relative py-20 px-6 text-center overflow-hidden flex flex-col items-center gap-4">
        <div className="w-3/5 h-auto">
        <p className='text-amber-300'>{qrCodeString}</p>
        ...
        <div className="px-8 py-3 bg-cyan-500 hover:bg-cyan-600 text-white font-semibold rounded-lg transition-colors shadow-lg shadow-cyan-500/50">
          ...
          <button className="flex flex-col items-center justify-center gap-2 text-center cursor-pointer"
            onClick={() => setIsPaused(!isPaused)}>
            <QrCode className="w-12 h-12 text-cyan-100" />
            <span>Scan QR Code</span>
          </button>
        </div>
          <button className='px-8 py-3 bg-cyan-500 hover:bg-cyan-600 text-white font-semibold rounded-lg' onClick={() => setIsPaused(true)}>Stop</button>
      </section>
    </div>
  )
}
